...
def init_loguru_handlers(log_pro_path: str = None,log_dir_name:str="logs",get_wsgi_logger:bool=False):
    '''
    ...
    if not log_pro_path:
        ...
        log_pro_path = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    # 定义info_log文件名称
    log_file_path = os.path.join(log_pro_path, log_dir_name,'info_{time:YYYYMMDD}.log')
    # 定义err_log文件名称
    err_log_file_path = os.path.join(log_pro_path, log_dir_name,'error_{time:YYYYMMDD}.log')

    from sys import stdout
    LOGURU_FORMAT: str = '<green>{time:YYYY-MM-DD HH:mm:ss.SSS}</green> | <level>{level: <16}</level> | {process.id} | <bold>{message}</bold>'
    # 这句话很关键避免多次的写入我们的日志
...
log = init_loguru_handlers(get_wsgi_logger=True)
